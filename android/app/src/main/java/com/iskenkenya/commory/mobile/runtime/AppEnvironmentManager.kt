...
class AppEnvironmentManager(context: Context) {
    ...
    @Volatile private var cachedSnapshot = AppEnvironment(serverUrl = defaultServerUrl())
    ...
    val environment: Flow<AppEnvironment> = dataStore.data.map { preferences ->
        AppEnvironment(
            ...
            serverUrl = preferences[KEY_SERVER_URL] ?: defaultServerUrl(),
    ...
    fun currentSnapshot(): AppEnvironment = cachedSnapshot
    ...
    private companion object {
        fun defaultServerUrl(): String {
            ...
            return if (isEmulator) "http://10.0.2.2:3000/" else "https://commory.iskenkenya.com/"
        }
