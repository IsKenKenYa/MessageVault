...
import com.iskenkenya.commory.mobile.ui.viewmodels.ServerAuthUiState
...
private fun ServerSettingsCard(
    ...
    var pendingMode by remember { mutableStateOf<RuntimeMode?>(null) }
    val message = authState.message.resolve()

    pendingMode?.let { mode ->
        AlertDialog(
            onDismissRequest = { pendingMode = null },
            title = { Text(stringResource(R.string.mode_switch_confirm_title)) },
            text = {
                Text(
                    if (mode == RuntimeMode.LOCAL_ONLY) {
                        stringResource(R.string.mode_switch_to_local_body)
                    } else {
                        stringResource(R.string.mode_switch_to_server_body)
                    }
                )
            },
            confirmButton = {
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            ...
            Row {
                FilterChip(
                    ...
                    onClick = {
                        if (environment.mode != RuntimeMode.LOCAL_ONLY) {
                            pendingMode = RuntimeMode.LOCAL_ONLY
                        }
                    },
                ...
                FilterChip(
                    ...
                    onClick = {
                        if (environment.mode != RuntimeMode.COMMORY_SERVER) {
                            pendingMode = RuntimeMode.COMMORY_SERVER
                        }
                    },
