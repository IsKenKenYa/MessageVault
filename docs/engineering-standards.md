...
- 手写业务源码硬上限为 `500` 行；现有治理脚本默认对当前变更集中的 Commory-owned 超线文件直接失败，避免把历史超线文件与本轮修复强绑定。
- Web 参考基线通过 `web/reference-baseline-paths.txt` 维护，覆盖从 `references/art-design-pro` 演化而来的路径前缀；这些 baseline 文件允许保留历史超线，但必须在复杂度报表里单列暴露。
...
- `bash scripts/check-repo-hygiene.sh` 会对当前变更集中的 Commory-owned 手写 `.go`、`.kt`、`.java`、`.ts`、`.tsx`、`.vue` 源码执行 `500` 行硬门禁，并按 `web/reference-baseline-paths.txt` 放行参考衍生的 Web baseline 文件。
- `bash scripts/report-loc-complexity.sh` 会输出手写源码最大文件列表、Commory-owned `350+` 警戒清单、Web baseline `350+` 清单，以及 `1000+` 的 generated/schema/resource 报告。
