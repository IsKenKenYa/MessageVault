...
## Why
MessageVault 组织下的 3 个仓库（.github、MessageVault-Mobile、SMS-Previewer）目前分散在独立仓库中，维护成本高、协作不便。将它们合并到当前工作区作为 Monorepo，推送到 git@github.com:IsKenKenYa/MessageVault.git，可以统一管理、简化 CI/CD 流程，并集中处理组织级文档和许可证。
...
#### Scenario: 许可证统一
- **WHEN** Monorepo 构建完成
- **THEN** 根目录包含 GPL v3.0 LICENSE，`mobile/` 和 `previewer/` 子目录也使用 GPL v3.0

### Requirement: 项目初始化与编译
系统 SHALL 对各子项目进行依赖安装和编译验证。

#### Scenario: Android 项目编译
- **WHEN** 对 `mobile/` 子目录执行 Gradle 构建
